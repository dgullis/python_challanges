...
def christmas_day_air_quality(filename, include_header_row):
    ...
    if include_header_row:
        return*get_header(filename), *christmas_day_contents(filename)
    else:
        return christmas_day_contents(filename)

def christmas_day_contents(filename):
        contents = [item for item in get_file_contents(filename) if item[0:5] == '25/12']
        return contents  
...
#[int(item[3]) for item in christmas_day_contents_split]

def get_christmas_day_air_values(filename):
    christmas_day_contents_air_values = [int(item.split(';')[3]) for item in christmas_day_contents(filename)]
    return christmas_day_contents_air_values

def christmas_day_average_air_quality(filename):
    average_air_quality = sum((get_christmas_day_air_values(filename))) / len(get_christmas_day_air_values(filename))
    return round(average_air_quality, 2)
