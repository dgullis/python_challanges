...
def lowest_squared(list):
   return sorted([num*num for num in list])[0]
...
def highest_squared(list):
    return sorted([num*num for num in list])[-1]
...
def starts_with_a_vowel(list):
    vowels = ['a', 'e', 'i', 'o', 'u']
    return [item for item in list if item[0] in vowels]
...
def greater_than(list, number):
    return [num for num in list if num > number]
...
def multiply_each_by(list, number):
    return [num*number for num in list]
